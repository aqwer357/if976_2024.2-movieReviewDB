...
CREATE TABLE U_Publico(
    USERNAME varchar(255) primary key references Pessoa(USERNAME) ON DELETE CASCADE
);
...
CREATE TABLE Tags(
    ID_FILME integer REFERENCES Filme(ID_FILME) ON DELETE CASCADE,
...
CREATE TABLE Avaliacao(
    ID_FILME integer REFERENCES Filme(ID_FILME) ON DELETE CASCADE,
    USERNAME varchar(255) REFERENCES U_Publico(USERNAME) ON DELETE CASCADE,
...
CREATE TABLE Tags_Solicitacao(
    ID_SOLICITACAO integer references Solicitacao_Filme(ID_SOLICITACAO) ON DELETE CASCADE,
    TAG varchar(255) NOT NULL,
...
CREATE TABLE Participou(
    ID_FILME integer REFERENCES Filme(ID_FILME) ON DELETE CASCADE,
    NOME varchar(255) REFERENCES Membro(NOME) ON DELETE CASCADE,
